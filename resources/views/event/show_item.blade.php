            ...
            <div class="item-orders">
                <ul>
                    @forelse($item->orders as $order)
                        <li {!! $order->participant->user->id === $user->id ? "id='my-contrib-$item->id'" : "" !!}><span class="item-order-username">{{$order->participant->user->name}}</span> prends <span class="item-order-value">{{$order->qty_taken}}</span></li>
                    @empty
